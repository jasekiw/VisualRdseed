        ...
        public void convert(bool fileready, bool outputready)
        {
            ...
            if (readytogo)
            {
                try
                {
                    ...
                    processInfo.Arguments = "-d -o " + fileFormat + " " + endtime + Poles + ExactResponses + Buffer + "-f \"/cygdrive/" + GrabDrive(inputFile) + "/" + RemoveDrivefromString(inputFile) + "\" -q \"/cygdrive/" + GrabDrive(outputFile) + "/" + RemoveDrivefromString(outputFile) + "\"";
